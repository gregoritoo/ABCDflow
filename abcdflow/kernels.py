...
def WN(x, y, sigma):
    ...
    w = tf.cast(tf.math.logical_not(tf.not_equal(tf.math.subtract(
        x2, y2), tf.constant(0, dtype=_precision)), name="inverse"), dtype=_precision)
    return sigma*w
...
def PER(x, y1, params):
    ...
    w = sigma * tf.math.exp(const_2*tf.math.square(
        tf.math.sin(const_1*tf.math.abs(tf.math.subtract(x2, y2)))))
    return w
...
def SE(x, y1, params):
    ...
    const_1 = tf.cast(0.5*tf.cast(-1/tf.math.square(l),
                      dtype=_precision), dtype=_precision)
...
def RQ(x, y, params):
    ...
    a = tf.cast(1/const*tf.math.add(tf.math.square(tf.math.subtract(y2, x2)),
                const_matrix), dtype=_precision)
...
def CP(x, y, params, left_kern, rigth_kern, left_params, rigth_params):
    ...
    left = tf.math.multiply(tf.math.multiply(
        neg_sig_y, neg_sig_x), left_kern(x, y, left_params))
    rigth = tf.math.multiply(tf.math.multiply(
        sig_y, sig_x), rigth_kern(x, y, rigth_params))
