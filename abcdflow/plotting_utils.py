...
def plot_gs_pretty(true_data, mean, X_train, X_s, stdp, stdi, color="blue"):
    ...
    try:
        ...
        pass
    true_data, X_train, X_s = true_data.reshape(
        -1), X_train.reshape(-1), X_s.reshape(-1)
...
def plot_gs(true_data, mean, X_train, X_s, stdp, stdi, color="blue"):
    ...
    plt.fill_between(X_s.reshape(-1,), stdp, stdi,
                     facecolor=CB91_Blue, alpha=0.2, label="Conf I")
...
def get_values(mu_s, cov_s, nb_samples=100):
    ...
    stdp = [np.mean(samples[:, i])+1.96*np.std(samples[:, i])
            for i in range(samples.shape[1])]
    stdi = [np.mean(samples[:, i])-1.96*np.std(samples[:, i])
            for i in range(samples.shape[1])]
