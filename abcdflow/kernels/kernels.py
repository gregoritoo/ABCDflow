...
@tf.function
def LIN(x: tf.Tensor, y: tf.Tensor, params: List) -> tf.Tensor:
    """
    ...
    x1 = tf.transpose(tf.math.subtract(
        x, c * tf.ones_like(x, dtype=_precision)))
...
@tf.function
def CONST(x: tf.Tensor, y: tf.Tensor, sigma: List) -> tf.Tensor:
    """
...
@tf.function
def WN(x: tf.Tensor, y: tf.Tensor, sigma: List) -> tf.Tensor:
    """
    ...
    w = tf.cast(
        tf.math.logical_not(
            tf.not_equal(tf.math.subtract(x2, y2),
                         tf.constant(0, dtype=_precision)),
            name="inverse",
        ),
        dtype=_precision,
    )
...
@tf.function
def PER(x: tf.Tensor, y1: tf.Tensor, params: List) -> tf.Tensor:
    """
    ...
    w = sigma * tf.math.exp(
        const_2
        * tf.math.square(tf.math.sin(const_1 * tf.math.abs(tf.math.subtract(x2, y2))))
    )
    return w


@tf.function
def SE(x: tf.Tensor, y1: tf.Tensor, params: List) -> tf.Tensor:
    """
    ...
    const_1 = tf.cast(
        0.5 * tf.cast(-1 / tf.math.square(l), dtype=_precision), dtype=_precision
    )
    return sigma * tf.math.exp(tf.math.square(tf.math.subtract(y2, x2)) * const_1)


@tf.function
def RQ(x: tf.Tensor, y: tf.Tensor, params: List) -> tf.Tensor:
    """
    ...
    a = tf.cast(
        1 / const *
        tf.math.add(tf.math.square(tf.math.subtract(y2, x2)), const_matrix),
        dtype=_precision,
    )
...
def sigmoid(x: tf.Tensor, s: float, cp: float) -> tf.Tensor:
    temp = cp * tf.ones_like(x, dtype=tf.float64)
    return tf.math.divide(
        tf.ones_like(x, dtype=tf.float64),
        tf.math.add(
            tf.ones_like(x, dtype=tf.float64),
            tf.math.exp(-s * tf.math.subtract(x, temp)),
        ),
    )


def dec_sigmoid(y: tf.Tensor, s: float, cp: float) -> tf.Tensor:
    one = tf.ones_like(y, dtype=tf.float64)
    temp = cp * tf.ones_like(y, dtype=tf.float64)
    return tf.math.subtract(
        one,
        tf.math.divide(
            tf.ones_like(y, dtype=tf.float64),
            tf.math.add(
                tf.ones_like(y, dtype=tf.float64),
                tf.math.exp(-s * tf.math.subtract(y, temp)),
            ),
        ),
    )


def tanh(x: tf.Tensor, s: float, cp: float) -> tf.Tensor:
    temp = cp * tf.ones_like(x, dtype=tf.float64)
    return 0.5 * tf.math.add(
        tf.ones_like(x, dtype=tf.float64), 1 / s * tf.math.subtract(temp, x)
    )


def dec_tanh(x: tf.Tensor, s: float, cp: float) -> tf.Tensor:
    ...
    return tf.math.subtract(
        ...
        0.5
        * tf.math.add(
            tf.ones_like(x, dtype=tf.float64), 1 /
            s * tf.math.subtract(temp, x)
        ),
    )
...
def CP(
    ...
    y: tf.Tensor,
    params: List,
    left_kern: Callable[[tf.Tensor, tf.Tensor, List], tf.Tensor],
    rigth_kern: Callable[[tf.Tensor, tf.Tensor, List], tf.Tensor],
    left_params: List,
    rigth_params: List,
) -> tf.Tensor:
    """
    ...
    left = tf.math.multiply(
        tf.math.multiply(neg_sig_y, neg_sig_x), left_kern(x, y, left_params)
    )
    rigth = tf.math.multiply(
        tf.math.multiply(sig_y, sig_x), rigth_kern(x, y, rigth_params)
    )
