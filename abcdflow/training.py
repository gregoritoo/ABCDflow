            ...
            try:
                ...
                if verbose:
                    for iteration in range(0, nb_iter):
                        ...
                        val = train_step(
                            model, iteration, X_train, Y_train, kernels_name, OPTIMIZER
                        )
                        if np.isnan(val):
                            loop += 1
                            break
                        sys.stdout.write(
                            "\r"
                            + "=" * int(iteration / nb_iter * 50)
                            + ">"
                            + "." * int((nb_iter - iteration) / nb_iter * 50)
                            + "|"
                            + " * log likelihood  is : {:.4f} at iteration : {:.0f} at epoch : {:.0f} / {:.0f} with lr of: {}".format(
                                val[0][0], iteration, loop + 1, nb_restart, lr
                            )
                        )
                        sys.stdout.flush()
                    sys.stdout.write("\n")
                    ...
                    for iteration in range(1, nb_iter):
                        val = train_step(
                            model, iteration, X_train, Y_train, kernels_name
                        )
                    loop += 1
...
def analyse(
    ...
    while len(COMB) > 1:
        ...
        if prune:
            ...
            BEST_MODELS, TEMP_BEST_MODELS = search_step(
                ...
                verbose,
                OPTIMIZER,
                initialisation_restart=initialisation_restart,
                GPY=GPY,
                mode=mode,
            )
    ...
    if not GPY:
        ...
        print(
            colored("[STATE]", "red"),
            "model BIC is {}".format(
                model.compute_BIC(X_train, Y_train, BEST_MODELS["model_list"])
            ),
        )
...
def single_model(
    ...
    """
    X_train, Y_train, X_s = (
        tf.convert_to_tensor(X_train, dtype=_precision),
        tf.convert_to_tensor(Y_train, dtype=_precision),
        tf.convert_to_tensor(X_s, dtype=_precision),
    )
    if reduce_data:
    ...
    BEST_MODELS = search_step(
        ...
        TEMP_BEST_MODELS=TEMP_BEST_MODELS,
...
def launch_analysis(
    ...
    """
    if prune and nb_by_step is None:
        raise ValueError("As prune is True you need to precise nb_by_step")
    if nb_by_step is not None and nb_by_step > loop_size:
    ...
    try:
        if experimental_multiprocessing:
            model, kernels = multithreaded_straight_analysis(
                X_train,
                Y_train,
                X_s,
                nb_restart,
            ...
            model, kernels = monothreaded_straight_analysis(
                X_train,
                Y_train,
                X_s,
                nb_restart,
...
@save_and_plot
def multithreaded_straight_analysis(
    X_train,
    Y_train,
    X_s,
    nb_restart,
    ...
    try:
        model, kernels = straigth_analyse(
            X_train,
            Y_train,
            X_s,
            nb_restart,
            ...
            GPY=GPY,
            mode=mode,
            parralelize_code=parralelize_code,
...
@save_and_plot
def monothreaded_straight_analysis(
    X_train,
    Y_train,
    X_s,
    nb_restart,
        ...
        if kernels_name[0] != "*":
            if not GPY:
                while (
                    ...
                    and failed < initialisation_restart * 2
                ):
                    try:
